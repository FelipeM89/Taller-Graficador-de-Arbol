...
def main():
    ...
    terminales = {simbolo for producciones in gramatica.values()
                  for rhs in producciones for simbolo in rhs if simbolo not in no_terminales}
    ...
    try:
        ...
        if analizador.pos == len(tokens):
            print("\n✅ Cadena aceptada. Árbol AST:\n")
            Graficador.imprimir_arbol(ast)
        else:
            print("❌ Cadena NO aceptada (tokens sin consumir).")
